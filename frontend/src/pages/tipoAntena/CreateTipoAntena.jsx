...
const CreateTipoAntenaFormSchema = yup.object().shape({
  nome: yup
    .string()
    .required("O nome do tipo de antena é obrigatório")
    .max(60, "O nome deve ter no máximo 60 caracteres"),
});

export const CreateTipoAntena = () => {
  ...
  const handleCreateTipoAntenaFormSchema = async (data) => {
    const newTipoAntena = {
      nome: data.nome.trim(),
    };
    setIsLoading(true);
    ...
    try {
      ...
      if (request.status == 201) {
        toast({
          title: "Tipo de antena criado com sucesso!",
          status: "success",
          position: "top-right",
          duration: 3000,
          isClosable: true,
  ...
  return (
    <Box
      ...
      borderRadius={8}
      p={["6", "8"]}
      onSubmit={handleSubmit(handleCreateTipoAntenaFormSchema)}
    >
      <Heading size="lg" fontWeight="500">
        Criar Tipo de Antena
      </Heading>
